...
def interchange(existing_lin, start_end_timestamps, nr_iterations, nr_swaps_stopping_criteria):
    ...
    enq_overlapping = init_overlapping(Op.Enq, start_end_timestamps)
    deq_overlapping = init_overlapping(Op.Deq, start_end_timestamps)
    start_t = datetime.datetime.now() # Start timing first iteration
    enq_pot_swaps = init_pot_swaps(start_end_timestamps, Op.Enq, enq_overlapping) # pot_swaps is item1:[(item2, re_imp)]
    deq_pot_swaps = init_pot_swaps(start_end_timestamps, Op.Deq, deq_overlapping)
    out_str = ""
    ...
    while count < nr_iterations: 
        ...
        # NOTE: May result in incorrect re_imps 
        (enq_pot_swaps, nr_enq_swaps) = execute_and_update(enq_pot_swaps, start_end_timestamps, Op.Enq, enq_overlapping) 
        (deq_pot_swaps, nr_deq_swaps) = execute_and_update(deq_pot_swaps, start_end_timestamps, Op.Deq, deq_overlapping)
        count += 1
        nr_swaps_this_iteration = nr_enq_swaps + nr_deq_swaps
        end_t = datetime.datetime.now()
        
        # Prepare output prints:
        tot_rank_error = get_total_rank_error(original_last_timestamp)
        tmp_out_str =  ("ITERATION " + str(count) + ":\n" +
                    "Nr enq swaps: " + str(nr_enq_swaps) + "\n" +
                    "Nr deq swaps: " + str(nr_deq_swaps) + "\n" +
...
'''
def interchange_partial(existing_lin, start_end_timestamps, nr_iterations, nr_swaps_stopping_criteria):
    nr_items_total = len(existing_lin)
    nr_items_to_process = 2000
    step = 1000
    count = 0
    iteration_count = 0
    original_last_timestamp = get_total_last_timestamp(start_end_timestamps)
    ...
    while iteration_count < nr_iterations:
        start_ind = count
        end_ind = count + nr_items_to_process
        while count < end_ind:
            
            old_interchange_inner(existing_lin, start_end_timestamps, nr_iterations, nr_swaps_stopping_criteria, start_ind, end_ind)
            count += step
...
def old_interchange_inner(existing_lin, start_end_timestamps, nr_iterations, nr_swaps_stopping_criteria, start_ind, end_ind):
...
def update_a_with_bs_list(item_a, item_b, pot_swaps, start_end_timestamps, op_type, overlapping_items):
    new_list = []
...
def update_other_a_lists_with_b(item_a, item_b, pot_swaps, start_end_timestamps, op_type, overlapping_items):
...
def init_pot_swaps(start_end_timestamps, op_type, overlapping_items):
...
def execute_and_update(pot_swaps, start_end_timestamps, op_type, overlapping_items):
    ...
    for item1, item1_list in pot_swaps.items():
        ...
        if best_imp[0] != None:
            ...
            pot_swaps = update_a_with_bs_list(item1, best_imp[0], pot_swaps, start_end_timestamps, op_type, overlapping_items)
            pot_swaps = update_a_with_bs_list(best_imp[0], item1, pot_swaps, start_end_timestamps, op_type, overlapping_items)

            # Update all other items' potential swap lists
            pot_swaps = update_other_a_lists_with_b(item1, best_imp[0], pot_swaps, start_end_timestamps, op_type, overlapping_items)
            pot_swaps = update_other_a_lists_with_b(best_imp[0], item1, pot_swaps, start_end_timestamps, op_type, overlapping_items)
